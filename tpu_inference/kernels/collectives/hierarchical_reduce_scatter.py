...
def _accumulate(
  ...
  load_recv_op = pltpu.make_async_copy(
      recv_hbm, vmem_recv.at[vmem_idx], load_recv_sem
  )
  load_run_op = pltpu.make_async_copy(
      run_hbm, vmem_run.at[vmem_idx], load_run_sem
  )
  load_recv_op.start()
  load_run_op.start()
  load_recv_op.wait()
  ...
  if out_hbm is not None:
    store_run_op = pltpu.make_async_copy(
        vmem_run.at[vmem_idx], out_hbm, store_run_sem
    )
  else:
    store_run_op = pltpu.make_async_copy(
        vmem_run.at[vmem_idx], run_hbm, store_run_sem
    )
          ...
          for hypercube_dim_idx in range(num_hypercube_dims):
            ...
            if k_size > 0:
              op = pltpu.make_async_remote_copy(
                  ...
                  device_id=neighbor_chip_id * 2 + cur_chiplet_bit,
                  device_id_type=pltpu.DeviceIdType.LOGICAL,
              )
              op.start()
              mb_ops.append((op, my_chunk_idx, mb_start_idx, k_size))

        all_mb_ops.append(mb_ops)

      for micro_batch_idx in range(num_micro_batches):
        store_ops = []
        for op_id, (op, my_chunk_idx, c_start, k_sz) in enumerate(
            all_mb_ops[micro_batch_idx]
        ):
          op.wait()
          s_op = _accumulate(
              recv_hbm=recv_buf_ref.at[
                  pl.ds(my_chunk_idx * final_chunk_size, final_chunk_size),
                  pl.ds(c_start, k_sz),
              ],
              ...
              out_hbm=None,
              vmem_recv=vmem_recv_ref.at[
                  :, pl.ds(0, final_chunk_size), pl.ds(0, k_sz)
              ],
              vmem_run=vmem_run_ref.at[
                  :, pl.ds(0, final_chunk_size), pl.ds(0, k_sz)
              ],
...
def hierarchical_reduce_scatter_local(
  ...
  out_shape = jax.ShapeDtypeStruct(
      (final_chunk_size, hidden_size_dim), local_x.dtype
  )
...
def hierarchical_reduce_scatter(
  ...
  def inner(local_x):
    return hierarchical_reduce_scatter_local(
        local_x, num_devices, num_micro_batches
    )
